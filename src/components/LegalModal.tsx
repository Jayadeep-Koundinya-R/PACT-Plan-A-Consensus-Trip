...
const PRIVACY_CONTENT = [
  ...
  { heading: '3. Private Constraints Stay Private', body: 'Your exact budget cap, specific available dates, and destination preferences are protected by database access policies and are visible only to the account and trusted consensus services. Other members are intended to see only aggregate results.' },
  ...
  { heading: '5. Data Storage & Security', body: 'Data is stored in Supabase PostgreSQL with row-level security policies. Authentication is handled via secure session tokens. AI prompts are processed through the authenticated AI service and are not intentionally retained by PACT.' },
  { heading: '6. Your Rights', body: 'You may: (a) request an export of your data, (b) clear locally cached account data from Settings, (c) revoke AI processing consent, and (d) request a data review. Server-side account deletion is not currently available in-app. Contact concierge@pact.travel for data requests.' },
...
const TERMS_CONTENT = [
  ...
  { heading: '6. Free Tier Limitations', body: 'The Free tier includes: (a) up to 1 active trip circle, (b) up to 8 members per circle, (c) 15 AI prompts per day, (d) basic consensus features. PACT reserves the right to adjust free tier limits with 30 days\' notice. Current limits are always displayed in-app.' },
  ...
  { heading: '9. Termination', body: 'We may suspend or terminate accounts that violate these terms. Settings can clear locally cached account data and sign you out. Contact concierge@pact.travel for server-side account removal requests.' },
...
const RULES_CONTENT = [
  ...
  { heading: 'Rule 2: Private Constraints, Public Overlap', body: 'Your inputs are protected by access policies and are not shown as raw values to other members. The group should see only aggregate overlap, budget bands, and destination results. PACT does not claim end-to-end encryption or mathematically impossible inference.' },
  ...
  { heading: 'Rule 4: Supermajority Consensus', body: 'A decision is confirmed only when 70% or more of members agree (supermajority threshold). This ensures broad buy-in without requiring unanimity, which often leads to deadlock in group travel planning.' },
  ...
  { heading: 'Rule 7: Keep Ballots Private', body: 'Individual ballot choices are restricted by access policies. The group experience should expose only aggregate results, not who voted for or against an option.' },
  ...
  { heading: 'Rule 10: Data Ownership', body: 'You control your use of PACT. You can request an export or clear locally cached data. Contact PACT for server-side data requests; permanent deletion is not currently completed by the Settings button.' }
];
